...
const { externalSamplingParams } = await import(
  "../src/features/chat/external-sampling.ts"
);
...
assert.ok(
  gatedSpreads.some((text) => text.includes("externalSamplingParams(")),
...
const buildSamplingFields = new Function(
  ...
  payload: typeof externalSamplingParams,
...
test("vLLM server default omits Min P while retaining other sampling fields", () => {
  ...
  assert.ok(!("min_p" in bodyFor("vllm", { ...PARAMS, minP: 0 })));
});
...
test("the panel and the request read the same capability flags", () => {
  ...
  const sampling = readSrc("features/chat/external-sampling.ts");
  assert.match(sampling, /capabilities\?\.minP/);
  assert.match(sampling, /capabilities\?\.repetitionPenalty/);
});
